...
export function* flatten(
  ...
  switch (node.type) {
    ...
    case "relative":
      ...
      break;
    case "compound":
      yield* node.list.map((token: Token): [Token, Compound] => [token, node]);
      break;
    default:
      yield [node, parent];
  }
...
export function specificity(selector: string | AST): number[] {
  ...
  if (ast.type === "list" && "list" in ast) {
    let base = 10;
    const specificities = ast.list.map((ast: string | AST) => {
      const sp = specificity(ast);
      base = Math.max(base, ...specificity(ast));
      return sp;
    });
    const numbers = specificities.map((ast: string | AST) =>
