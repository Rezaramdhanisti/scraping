...
import { makeWaitForNextTask } from "../utils.js";
import { httpHappyEyeballsAgent, httpsHappyEyeballsAgent } from "../utils.js";
...
export class WebSocketTransport implements ConnectionTransport {
  ...
  static async connect(
    ...
    return await WebSocketTransport._connect(
      ...
      url,
      headers || {},
      { follow: !!followRedirects, hadRedirects: false },
      debugLogHeader,
    );
  }
  ...
  static async _connect(
    ...
    url: string,
    headers: { [key: string]: string },
    redirect: { follow: boolean; hadRedirects: boolean },
    debugLogHeader?: string,
  ): Promise<WebSocketTransport> {
  ...
  constructor(
    ...
    this._ws.addEventListener("error", (error) =>
      this._progress?.log(
        `<ws error> ${logUrl} ${error.type} ${error.message}`,
      ),
    );
  }
  ...
  async closeAndWait() {
    if (this._ws.readyState === ws.CLOSED) return;
